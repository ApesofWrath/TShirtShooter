...
const double MAX_Y_RPM_ACTUAL = 950.0;
const double MAX_Y_RPM = 600.0;
const double MAX_ROT_VEL_ACTUAL = 30.0;
...
void DriveController::Drive(Joystick *joyThrottle, Joystick *joyWheel) {
	...
	current_l = (canTalonFrontLeft->GetEncVel() / UNITS_PER_ROT) * MINUTE_CONVERSION;
	current_r = (canTalonFrontRight->GetEncVel() / UNITS_PER_ROT) * MINUTE_CONVERSION; //right is negative for forward
	...
	canTalonFrontLeft->Set(-output_l); //backs are slaved to fronts
